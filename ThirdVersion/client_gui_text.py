#!/usr/bin/env python3
"""
client_gui_text.py - VERSIÓN FINAL CORREGIDA
"""
...
class GridBoard(tk.Canvas):
    def __init__(self, parent, size=10, cell=40, pad=6, title=None, *args, **kwargs):  # AUMENTADO cell=40
        self.label_size = 20
...
class App(tk.Tk):
    ...
    def poll_queue(self):
        while not msgq.empty():
            ...
            # PROCESAR SERVER/RESULT - CORRECCIÓN FINAL
            if topic.startswith(f"game/{ROOM}/server/"):
                ...
                if d.get("result") == "start":
                    self.append_status("🎮 JUEGO INICIADO - Esperando primer turno...")
                    next_turn = d.get("next_turn")
                    if next_turn:
                        self.append_status(f"Primer turno: {next_turn}")
                        if next_turn == PLAYER:
                            self.append_status("🎯 ¡ES TU TURNO INICIAL!")
                            self.my_turn = True
                        else:
                            self.append_status("⏳ El oponente comienza...")
                            self.my_turn = False
                    continue
                    ...
                    if error_msg == "not_your_turn":
                        ...
                        if self.last_shot:
                            try:
                                ...
                                self.append_status(f"Limpiando disparo no permitido en {self.last_shot}")
                            except Exception:
                                pass
                            self.last_shot = None
                    continue
                ...
                # Si faltan campos críticos, mostrar advertencia pero continuar
                if not all([attacker, target, coord, res]):
                    self.append_status(f"⚠️ Mensaje server/result con campos incompletos: {d}")
                    # Intentar procesar con los campos que sí tenemos
                    if coord and res:
                ...
                # Limpiar last_shot si fue procesado
                if self.last_shot == coord:
                    self.last_shot = None

                # Información de turno
                next_turn = d.get("next_turn") or d.get("next turn")
                if next_turn:
                    self.append_status(f"🔄 Turno cambiado a: {next_turn}")
                    if next_turn == PLAYER:
                        self.append_status("🎮 ¡ES TU TURNO!")
                        self.my_turn = True
                    else:
                        self.append_status("⏳ Espera tu turno...")
                        self.my_turn = False
            ...
            # PROCESAR BOARD - PARA MOSTRAR BARCOS E IMPACTOS RECIBIDOS
