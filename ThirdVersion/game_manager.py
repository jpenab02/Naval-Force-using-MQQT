...
def coord_to_index(coord: str) -> Tuple[int,int]:
    ...
    # CORRECCIÓN: Letra representa columna, número representa fila
    ...
    r = row - 1          # Fila: 1=0, 2=1, etc.
    c = ROW_INDEX[col_char]  # Columna: A=0, B=1, etc.
...
def handle_move(self, room_id: str, player_id: str, coord: str) -> Dict:
    ...
    # **CORRECCIÓN CRÍTICA: Lógica de turnos invertida**
    # REGLA CORRECTA: Si aciertas -> sigues tú, si fallas -> cambia turno
    if res.get("result") == "miss":
        room["turn"] = opp  # Solo cambia turno si falla
    # Si es hit, el turno se queda en el mismo jugador
    ...
    out = {
        ...
        "next_turn": room["turn"],  # Este refleja el turno actualizado (o no)
        "victory": victory
    }
    return out
