...
def on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        logging.info("✅ [SERVER] Conectado al broker MQTT correctamente")
        
        # Suscribirse explícitamente a todos los topics necesarios
        topics = [
            (TOPIC_MOVES, 1),
            (f"game/+/server/result", 1),
            (f"game/+/player/+/board", 1)
        ]
        
        for topic, qos in topics:
            try:
                client.subscribe(topic, qos=qos)
                logging.info(f"✅ [SERVER] Suscrito a: {topic} (QoS: {qos})")
            except Exception as e:
                logging.error(f"❌ [SERVER] Error suscribiéndose a {topic}: {e}")
        
    else:
        logging.error("❌ [SERVER] Error de conexión al broker, rc=%s", rc)

def on_message(client, userdata, msg):
    try:
        payload = msg.payload.decode("utf-8")
        logging.info("🔴 [SERVER] MENSAJE RECIBIDO topic=%s", msg.topic)
        data = json.loads(payload)
    except Exception as e:
        logging.exception("❌ [SERVER] Payload inválido o no JSON: %s", e)
        return

    # Extraer room y player del topic
    parts = msg.topic.split('/')
    
    # Verificar si es un mensaje de movimiento
    if len(parts) >= 5 and parts[0] == "game" and parts[2] == "player" and parts[4] == "move":
        room = parts[1]
        player_id = parts[3]
        coord = data.get("coordinate")
        
        logging.info(f"🔴 [SERVER] Procesando MOVIMIENTO: room={room}, player={player_id}, coord={coord}")
        
        if coord is None:
            logging.warning("❌ [SERVER] Payload sin campo 'coordinate'")
            return

        # Verificar si la sala existe
        if room not in gm.rooms:
            logging.warning("❌ [SERVER] Room %s no encontrada. Salas existentes: %s", room, list(gm.rooms.keys()))
            err = {"error": "room_not_found", "room": room}
            client.publish(PUB_RESULT_TEMPLATE.format(room=room), json.dumps(err), qos=1)
            return

        logging.info(f"🟡 [SERVER] Sala {room} encontrada. Llamando a handle_move...")
        
        try:
            # Procesar jugada
            result = gm.handle_move(room, player_id, coord)
            logging.info(f"🟡 [SERVER] Resultado de handle_move: {result}")
        except Exception as e:
            logging.exception("❌ [SERVER] Error en handle_move: %s", e)
            result = {"error": "server_error", "message": str(e)}
        ...
        try:
            ...
            if next_turn:
                if room in gm.rooms and "players_list" in gm.rooms[room]:
                    ...
                    logging.info("✅ [SERVER] Turno aplicado: %s -> %s", room, next_turn)
        except Exception as e:
            logging.exception("❌ [SERVER] Error aplicando next_turn: %s", e)

        # Publicar resultado general
        out_topic = PUB_RESULT_TEMPLATE.format(room=room)
        out_payload = json.dumps(result)
        logging.info(f"🟡 [SERVER] PUBLICANDO RESULTADO en {out_topic}")
        logging.info(f"🟡 [SERVER] Payload: {out_payload}")
        
        try:
            client.publish(out_topic, out_payload, qos=1)
            logging.info("✅ [SERVER] Resultado publicado correctamente")
        except Exception as e:
            logging.exception("❌ [SERVER] Error publicando resultado: %s", e)
        ...
        try:
            ...
            if "error" not in result:  # Solo publicar tableros si no hay error
                # Tablero del atacante
                attacker_board = gm.get_board_summary(room, attacker, reveal=True)
                attacker_topic = PUB_BOARD_TEMPLATE.format(room=room, player_id=attacker)
                client.publish(attacker_topic, json.dumps(attacker_board), qos=1)
                logging.info(f"✅ [SERVER] Tablero publicado para atacante {attacker}")
                
                # Tablero del defensor
                if defender:
                    defender_board = gm.get_board_summary(room, defender, reveal=True)
                    defender_topic = PUB_BOARD_TEMPLATE.format(room=room, player_id=defender)
                    client.publish(defender_topic, json.dumps(defender_board), qos=1)
                    logging.info(f"✅ [SERVER] Tablero publicado para defensor {defender}")
                    
        except Exception as e:
            logging.exception("❌ [SERVER] Error publicando tableros: %s", e)

        save_state()
    
    else:
        logging.debug("[SERVER] Tópico no procesado: %s", msg.topic)
...
def http_create_room():
    ...
    logging.info(f"🔵 [HTTP] Creando sala: {room} con jugadores: {players}")
    
    if not room or not players or len(players) < 2:
        return jsonify({"error":"room and at least 2 players required"}), 400
    
    if room in gm.rooms:
        logging.warning(f"🔵 [HTTP] La sala {room} ya existe")
        return jsonify({"error":"room_exists"}), 400
    
    try:
        result = gm.create_room(room, players)
        logging.info(f"✅ [HTTP] Sala creada: {result}")
        save_state()
        
        # Publicar tableros vía MQTT
        for pid in players:
            publish_board_for_player(mqtt_client_global, room, pid)
            logging.info(f"✅ [HTTP] Tablero publicado para {pid}")
        ...
        logging.info(f"✅ [HTTP] Mensaje de inicio publicado: {start_msg}")

        return jsonify({"ok": True, "room": room, "players": players}), 201

    except Exception as e:
        logging.exception("❌ [HTTP] Error creando sala: %s", e)
